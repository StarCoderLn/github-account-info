...
- [x] T-506：优化监控仪表盘视觉层级、健康概览、单点趋势说明与响应式布局。
- [x] T-507：增加 10 秒后台刷新，并把 processor 部署参数调整为常驻 1 个任务的
  准实时模式；保留切回 `DesiredCount=0` 的低成本能力。
...
- [ ] T-709：合并到 `master` 后以 reviewed Change Set 部署
  `DesiredCount=1`，验证真实浏览器事件在约 10–20 秒内出现在统计页面。
