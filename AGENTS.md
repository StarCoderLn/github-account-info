...
- [F6-L08] `execute-change-set` 返回成功不等于 Stack 部署成功：GitHub Actions 必须先读取 `ChangeSetType`，执行后分别等待 `stack-create-complete` 或 `stack-update-complete`，否则 CloudFormation 已进入回滚时 workflow 仍会误报 Success。CloudWatch Logs 的创建权限还要同时覆盖固定 Log Group 的精确 ARN 与 `:*` 子资源 ARN；失败栈清理必须单独手工触发，并只允许 `ROLLBACK_COMPLETE`/`CREATE_FAILED`，禁止泛化为任意 Stack 删除。
