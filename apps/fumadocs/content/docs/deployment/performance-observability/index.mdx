...
Web 接入没有直接在入口静态 import SDK，而是在 React 首屏挂载后使用
`requestIdleCallback` 动态加载监控 chunk。配置来自：
...
| 服务 | 当前成本特征 |
| --- | --- |
| ECS Fargate | Processor 从 0 到 1 运行期间按 vCPU/内存秒计费，空闲为 0 时无常驻 Task 计算费 |
...
因此最需要避免的是把 Processor 长期固定为 `DesiredCount=1`。当前设计通过
`MinCapacity=0`、`MaxCapacity=1` 控制上限，同时接受 Fargate 冷启动带来的分钟级
可见延迟。
...
<Callout type="warn" title="202 不等于页面立即有数据">
  `202 Accepted` 只证明浏览器到 SQS 的接收链路成功。如果生产 Processor 当前为
  `DesiredCount=0` 且队列自动伸缩增强尚未部署，消息会留在 SQS，页面不会立即出现
  新统计。必须等 Processor 实际消费并写入 PostgreSQL 后再刷新页面。
</Callout>
